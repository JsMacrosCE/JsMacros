package com.jsmacrosce.doclet.tsdoclet.parsers;

import com.sun.source.doctree.*;
import org.jetbrains.annotations.NotNull;
import com.jsmacrosce.StringHelpers;
import com.jsmacrosce.doclet.*;
import com.jsmacrosce.doclet.tsdoclet.Main;
import com.jsmacrosce.doclet.tsdoclet.PackageTree;
...
import static com.jsmacrosce.doclet.tsdoclet.PackageTree.tsReservedWords;
...
public abstract class AbstractParser {
    ...
    public String transformType(TypeMirror type, boolean isParamType, boolean isExtends) {
        switch (type.getKind()) {
            ...
            case DECLARED -> {
                ...
                if (classpath.startsWith("com.jsmacrosce.")) {
                ...
                // check BaseEvent
                if (res.equals("com.jsmacrosce.jsmacros.core.event.BaseEvent")) {
    ...
    private String convertSignature(String sig) {
        if (sig.matches("^com\\.jsmacrosce\\.[^#]+\\w$")) return sig.replaceFirst("^.+\\.(?=[^.]+$)", "");
        if (sig.matches("^\\w+\\.(?:\\w+\\.)+[\\w$_]+$")) return "Packages." + sig;
//        sig = sig.replaceFirst("(?<=\\S)(?=[<(])", " ");
        return sig.startsWith("#")
            ? sig.substring(1)
            : sig.replaceFirst("^(?:com\\.jsmacrosce\\.jsmacros\\.(?:client\\.api|core)\\.library\\.impl\\.)?F([A-Z]\\w+)#", "$1.").replaceFirst("#", ".");
    }
