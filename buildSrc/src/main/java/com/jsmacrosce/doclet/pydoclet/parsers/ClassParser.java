package com.jsmacrosce.doclet.pydoclet.parsers;

import com.sun.source.doctree.*;
import com.sun.source.util.DocTreePath;
import com.jsmacrosce.doclet.pydoclet.Main;
...
public class ClassParser {
    ...
    private String createDescription(Element element, List<? extends DocTree> inlineDoc) {
        ...
        for (DocTree docTree : inlineDoc) {
            switch (docTree.getKind()) {
                ...
                case START_ELEMENT -> {
                    if (Objects.equals(docTree.toString(), "<li>")) {
                        sb.append("\n - ");
                    }
                    if (Objects.equals(docTree.toString(), "<p>")) {
                        sb.append("\n");
                    }
                }
            }
        ...
        imports.forEach(t -> {
            if (!types.containsKey(getClearedNameFromTypeMirror(t))) {
                if (!withArg.containsKey(getClearedNameFromTypeMirror(t))) {
                    if ((t + "").startsWith("com.jsmacrosce")) {
    ...
    private String parseDeclared(TypeMirror type, boolean cls) {
        ...
        if (getClearedNameFromTypeMirror(this.type.asType()).equals(getClearedNameFromTypeMirror(type))) {
            return "\"" + getClassName((TypeElement) Main.typeUtils.asElement(type)) + "\"";
        }

        if (withArg.containsKey(getClearedNameFromTypeMirror(type))) {
            if (withArg.get(getClearedNameFromTypeMirror(type)).contains("List")) {
                importList = true;
            } else if (withArg.get(getClearedNameFromTypeMirror(type)).contains("Mapping")) {
                importMapping = true;
            } else if (withArg.get(getClearedNameFromTypeMirror(type)).contains("Set")) {
                importSet = true;
            }
